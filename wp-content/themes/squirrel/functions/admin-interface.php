...
function squirrel_optionsframework_add_admin() {
    global $query_string;

    $themename = squirrel_get_option('of_themename');
    $shortname = squirrel_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = squirrel_get_option('of_template');
            squirrel_reset_options($options, 'optionsframework');
    ...
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'squirrel_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'squirrel_load_only');
}

add_action('admin_menu', 'squirrel_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function squirrel_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        squirrel_delete_option('of_options');
    }
}
...
function squirrel_optionsframework_options_page() {
    $options = squirrel_get_option('of_template');
    $themename = squirrel_get_option('of_themename');
    ?>
    <div class="trail-notify">
        <h1><?php _e('Go Squirrel Theme PRO!', 'squirrel'); ?></h1>
        <p><?php _e('You are Using the Lite Version of Squirrel Theme. Upgrade to Pro for extra features, Home Page Slider, Contact Page Template and more. </p>
        <a href="http://www.inkthemes.com/wp-themes/squirrel-wordpress-theme/" target="blank">Upgrade to Squirrel Theme PRO →</a>', 'squirrel'); ?>
    ...
    <div class="wrap" id="of_container">
        ...
        <form action="" enctype="multipart/form-data" id="ofform">
            ...
            $return = squirrel_optionsframework_machine($options);
            ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = squirrel_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = squirrel_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - squirrel_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'squirrel_ajax_callback');

function squirrel_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        squirrel_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = squirrel_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            squirrel_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    squirrel_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    squirrel_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            squirrel_update_option($multicheck_id, 'false');
                        } else {
                            squirrel_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    squirrel_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    squirrel_update_option($id, $border_array);
...
function squirrel_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = squirrel_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'textarea':
                ...
                $output .= '<textarea class="of-input" name="' . esc_attr($value['id']) . '" id="' . esc_attr($value['id']) . '" cols="' . $cols . '" rows="8">' . $ta_value . '</textarea>';
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= squirrel_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= squirrel_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = squirrel_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
                $output .= '<div id="' . esc_attr($value['id']) . '_picker" class="colorSelector"><div></div></div>';
                $output .= '<input class="of-color" name="' . esc_attr($value['id']) . '" id="' . esc_attr($value['id']) . '" type="text" value="' . $val . '" />';
                break;

            case "typography":

                $default = $value['std'];
                $typography_stored = squirrel_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = squirrel_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = squirrel_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - squirrel_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function squirrel_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = squirrel_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (squirrel_get_option($id) != "") {
            $val = squirrel_get_option($id);
        }
