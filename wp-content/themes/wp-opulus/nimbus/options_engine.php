...
function nimbus_field_engine() {
    ...
    foreach ($options as $option) {
        ...
        if ($option['type'] == "text") {
            ...
            $join_text = __('This feature is available to Nimbus Themes members.', 'nimbus');
            $join_link_text = __('Join today!!.', 'nimbus');
            $section_return[$option['tab']] .= '<p><span style="color:#fc7e2a;">' . $join_text . '</span> <a href="http://www.nimbusthemes.com/?utm_source=wp_opulus&utm_medium=theme&utm_content=panel_link&utm_campaign=opulus">' . $join_link_text . '</a></p>';
            $field_num++;    
